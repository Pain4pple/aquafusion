...
class turbidityProvider with ChangeNotifier {
  ...
  turbidityProvider(this._mqttStreamService) {
    _mqttStreamService.turbidityStream.listen((turbidity) {
      ...
      _turbidityHistory.add(ChartData(DateTime.now(), turbidity)); // Add new data to history
      if (_turbidityHistory.length > 50) _turbidityHistory.removeAt(0); // Limit history size
      print("Turbidity Data: $_turbidity"); // Debugging line to check data flow
      notifyListeners();
    });
  }
